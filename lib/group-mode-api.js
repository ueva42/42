...
export function registerGroupModeRoutes(app, deps) {
  ...
  async function busyStudentIds(classId, schoolId, sessionDate, excludeSessionId = null) {
    const date = toIsoDateOnly(sessionDate);
    if (!date) return new Set();
    const params = [classId, date];
    let schoolFilter = "";
    if (schoolId != null) {
      params.push(schoolId);
      schoolFilter = ` AND gs.school_id = $${params.length}`;
    }
    let exclude = "";
    if (excludeSessionId) {
  ...
  app.post("/api/teacher/group-sessions/:id/delete", isAdmin, async (req, res) => {
    try {
      ...
      await pool.query(
        ...
        WHERE id = $1 AND (school_id = $2 OR $2::int IS NULL)
      `,
        [req.params.id, schoolId]
      );
    ...
    try {
      ...
      for (const row of activeRes.rows) {
        try {
          ...
          console.error("⚠️ bootstrap session skip:", row.id, sessionErr);
        }
      }
  ...
  app.patch("/api/student/group-sessions/:id/members", isStudent, async (req, res) => {
    try {
      ...
      const sessionDate =
        toIsoDateOnly(access.bundle.session.sessionDate) || todayIsoDate();
      const busy = await busyStudentIds(
        access.classId,
        access.schoolId,
        sessionDate,
      ...
      // Nur über class_id prüfen – school_id kann bei Schüler:innen NULL/abweichend sein
      ...
      for (const id of memberIds) {
        ...
        if (busy.has(Number(row.id)) || busy.has(row.id)) {
          return res.json({
            success: false,
      ...
      const roles = (bundle.settings?.roles || []).filter((r) => r.active !== false);
      res.json({
        success: true,
        ...bundle,
      ...
      res.status(500).json({
        error: "Serverfehler",
        message: "Die Gruppenmitglieder konnten nicht gespeichert werden."
      });
    }
  });
  ...
  async function deleteGroupSessionForStudent(req, res) {
    try {
      ...
      // Host oder Mitglied (oder Setup ohne Mitglieder) darf löschen
      if (!isHost && !isMember && access.bundle.members.length > 0) {
        return res.status(403).json({ error: "Nur die Gruppe kann diese Arbeit löschen." });
      }

      await pool.query("DELETE FROM group_sessions WHERE id = $1", [req.params.id]);
      res.json({ success: true });
    } catch (err) {
