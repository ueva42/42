...
export function registerGroupModeRoutes(app, deps) {
  ...
  app.put("/api/teacher/group-mode/settings", isAdmin, async (req, res) => {
    try {
      ...
      const maxWhat = Math.max(1, Math.min(5, Number(req.body.maxWhatGoals) || 3));
      const maxHow = Math.max(1, Math.min(5, Number(req.body.maxHowGoals) || 3));
  ...
  app.put("/api/student/group-sessions/:id/member-goals", isStudent, async (req, res) => {
    try {
      ...
      const maxWhat = Math.max(1, Math.min(5, Number(settings.maxWhatGoals) || 3));
      const maxHow = Math.max(1, Math.min(5, Number(settings.maxHowGoals) || 3));
      ...
      async function resolveGoalList(rawList, singleId, singleText, type, max) {
        ...
        if (incoming && incoming.length) {
          for (const item of incoming.slice(0, max)) {
