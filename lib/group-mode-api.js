...
export function registerGroupModeRoutes(app, deps) {
  ...
  app.put("/api/student/group-sessions/:id/member-goals", isStudent, async (req, res) => {
    try {
      ...
      // Pro Person: mehrere Ziele, höchstens 3 verschiedene
      const maxWhat = Math.min(3, Math.max(1, Number(settings.maxWhatGoals) || 3));
      const maxHow = Math.min(3, Math.max(1, Number(settings.maxHowGoals) || 3));
