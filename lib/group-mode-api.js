...
export function registerGroupModeRoutes(app, deps) {
  ...
  app.get("/api/teacher/group-mode/role-goals/sample.csv", isAdmin, async (_req, res) => {
    res.setHeader("Content-Type", "text/csv; charset=utf-8");
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="rollen-und-rollenziele-beispiel.csv"'
    );
    res.send("\uFEFF" + ROLE_GOALS_CSV_EXAMPLE);
  });

  app.post("/api/teacher/group-mode/role-goals/import/preview", isAdmin, async (req, res) => {
    try {
      const schoolId = req.session.user.school_id;
      const classId = Number(req.body.classId);
  ...
  app.post("/api/teacher/group-mode/role-goals/import/confirm", isAdmin, async (req, res) => {
    try {
      const schoolId = req.session.user.school_id;
      const classId = Number(req.body.classId);
      const subject = resolveSubject(req.body.subject) || String(req.body.subject || "").trim();
      ...
      for (const row of parsed.rows) {
        ...
        // merge: add if missing; add_new: same for new
        const orderRes = await pool.query(
          `
          SELECT COALESCE(MAX(sort_order), 0) + 1 AS n
          FROM group_mode_role_goals WHERE role_id = $1 AND goal_type = $2
  ...
  app.put("/api/student/group-sessions/:id/member-goals", isStudent, async (req, res) => {
    try {
      ...
      let howGoalId = req.body.howGoalId || null;
      ...
      if (settings.enableWhatGoals) {
        if (whatGoalId) {
          ...
          if (!found) {
            return res.json({
              success: false,
              message: "Bitte wähle ein Rollen-Was-Ziel zu deiner Aufgabe."
            });
          }
          whatGoalText = found.goal.text;
          whatGoalArea = found.role.name;
          if (found.goal.level) {
            selectedLevel = found.goal.level;
            levelGoalText = found.goal.explanation || null;
          } else {
            selectedLevel = null;
            levelGoalText = found.goal.explanation || null;
          }
          ...
          return res.json({
            success: false,
            message: "Bitte wähle, was heute dein Beitrag in deiner Rolle ist."
          });
        }
      }
      ...
      if (settings.enableHowGoals) {
        if (howGoalId) {
          const foundHow = await findRoleGoalForMember(
            member,
            howGoalId,
            "WIE",
            settings.roles || []
          ...
          if (!foundHow) {
            return res.json({
              success: false,
              message: "Bitte wähle ein Rollen-Wie-Ziel zu deiner Aufgabe."
            });
          }
          howGoalText = foundHow.goal.text;
        } else if (!howGoalText) {
          return res.json({
            success: false,
            message: "Bitte wähle, wie du deinen Beitrag umsetzen möchtest."
          });
        } else {
          ...
          for (const role of settings.roles || []) {
            if (!assignedIds.has(String(role.id))) continue;
            for (const g of role.howGoals || []) {
              if (g.active !== false) roleHowTexts.add(g.text);
            }
          }
          const allowed =
            roleHowTexts.has(howGoalText) ||
