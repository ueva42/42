...
export function registerGroupModeRoutes(app, deps) {
  ...
  async function deleteSessionCascade(sessionId) {
    ...
    try {
      ...
      await client.query("DELETE FROM group_session_docs WHERE session_id = $1", [sessionId]);
      await client.query("DELETE FROM group_competency_activity WHERE session_id = $1", [
        sessionId
      ]);
      await client.query("DELETE FROM group_sessions WHERE id = $1", [sessionId]);
  ...
  app.post("/api/student/group-sessions", isStudent, async (req, res) => {
    try {
      ...
      // Themengebunden: offene Gruppe fortsetzen statt neue Tages-Session
