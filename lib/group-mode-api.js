...
export function registerGroupModeRoutes(app, deps) {
  ...
  async function getSettingsRow(schoolId, classId, subject) {
    const r = await pool.query(
      ...
      WHERE school_id = $1 AND class_id = $2 AND subject = $3
      LIMIT 1
    `,
      [schoolId, classId, subject]
    );
    return r.rows[0] || null;
  }

  async function getOrCreateSettings(schoolId, classId, subject) {
    let row = await getSettingsRow(schoolId, classId, subject);
    if (row) {
      await ensureDefaultRolesForSettings(pool, schoolId, row.id);
      return row;
    }
    ...
    row = ins.rows[0];
    const seedRoles =
      String(subject).toLowerCase() === "physik"
        ? DEFAULT_PHYSICS_ROLES
        : DEFAULT_PHYSICS_ROLES;
    await ensureDefaultRolesForSettings(pool, schoolId, row.id, seedRoles);
    return row;
  }
  ...
  async function loadSessionBundle(sessionId, schoolId) {
    ...
    const docsRes = await pool.query(
      `
      SELECT id, doc_type, content, created_by, created_at
  ...
  async function assertStudentCanAccessSession(studentId, sessionId) {
    ...
    if (!bundle || bundle.session.classId !== classId) {
      return { ok: false, status: 404, error: "Gruppe nicht gefunden." };
    }
    const isMember = bundle.members.some((m) => m.userId === studentId);
    const isHost = bundle.session.hostUserId === studentId;
    if (!isMember && !isHost && bundle.session.status !== "setup") {
      // During setup, any classmate can continue creating if they are host;
      // after setup, only members/host.
      return { ok: false, status: 403, error: "Du gehörst nicht zu dieser Gruppe." };
    }
    if (!isMember && !isHost && bundle.session.status === "setup") {
      // Only host may edit setup after creation
      if (bundle.session.hostUserId && bundle.session.hostUserId !== studentId) {
        return { ok: false, status: 403, error: "Diese Gruppe wird schon eingerichtet." };
      }
    }
    return { ok: true, bundle, classId, schoolId };
  }

  async function busyStudentIds(classId, schoolId, sessionDate, excludeSessionId = null) {
    const params = [classId, schoolId, sessionDate];
    let exclude = "";
    if (excludeSessionId) {
  ...
  app.get("/api/teacher/group-mode/settings", isAdmin, async (req, res) => {
    try {
      ...
      if (!classId || !subject || !LOG_SUBJECTS.includes(subject)) {
  ...
  app.get("/api/teacher/group-sessions", isAdmin, async (req, res) => {
    try {
      ...
      const params = [schoolId, classId, date];
      let subjectFilter = "";
      if (subject) {
        params.push(subject);
        subjectFilter = ` AND gs.subject = $${params.length}`;
      }
      ...
      const list = await pool.query(
        ...
        SELECT gs.id
        FROM group_sessions gs
        WHERE gs.school_id = $1 AND gs.class_id = $2 AND gs.session_date = $3
        ${subjectFilter}
  ...
  app.post("/api/student/group-sessions", isStudent, async (req, res) => {
    try {
      ...
      const ins = await pool.query(
        ...
        VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'setup','members',$9,$9)
        RETURNING id
      `,
        [
          schoolId,
      ...
      const bundle = await loadSessionBundle(ins.rows[0].id, schoolId);
      res.json({ success: true, ...bundle });
    } catch (err) {
  ...
  app.get("/api/student/group-sessions/:id", isStudent, async (req, res) => {
    try {
      ...
      const busy = await busyStudentIds(
        access.classId,
        access.schoolId,
        access.bundle.session.sessionDate,
      ...
      const studentsRes = await pool.query(
        ...
        ORDER BY name ASC
      `,
        [access.classId, access.schoolId]
      );
      ...
      if (access.bundle.session.subject) {
        ...
        topics = checks
          .filter((c) => c.subject === access.bundle.session.subject)
          .map((c) => ({
            id: c.id,
            name: c.name,
            goals: (c.goals || []).map((g) => ({
  ...
  app.patch("/api/student/group-sessions/:id/members", isStudent, async (req, res) => {
    try {
      ...
      const roster = await pool.query(
        ...
        WHERE role = 'student' AND class_id = $1 AND school_id = $2 AND id = ANY($3::int[])
      `,
        [access.classId, access.schoolId, memberIds]
      );
      if (roster.rows.length !== memberIds.length) {
        return res.json({ success: false, message: "Einige Personen gehören nicht zu eurer Klasse." });
      }
      for (const row of roster.rows) {
        if (busy.has(row.id)) {
          return res.json({
            success: false,
            message: `${displayNameFromUser(row.name)} ist heute schon in einer anderen Gruppe.`
          });
        }
      }

      await pool.query("DELETE FROM group_session_members WHERE session_id = $1", [
        access.bundle.session.id
      ]);

      let order = 1;
      for (const id of memberIds) {
        const user = roster.rows.find((r) => r.id === id);
        await pool.query(
          `
          INSERT INTO group_session_members (session_id, user_id, display_name_snapshot, sort_order)
          VALUES ($1, $2, $3, $4)
        `,
          [access.bundle.session.id, id, displayNameFromUser(user.name), order++]
        );
      }
      ...
      res.json({
        ...
        suggestedAssignments: suggestRoleAssignment(
          memberIds,
          bundle.settings.roles.filter((r) => r.active)
        )
      });
    } catch (err) {
  ...
  app.patch("/api/student/group-sessions/:id/roles", isStudent, async (req, res) => {
    try {
      ...
      const roles = access.bundle.settings.roles.filter((r) => r.active);
