...
export function registerGroupModeRoutes(app, deps) {
  ...
  async function assertStudentCanAccessSession(studentId, sessionId) {
    ...
    if (!isMember && !isHost) {
      if (bundle.session.status === "setup" && bundle.session.hostUserId) {
        return { ok: false, status: 403, error: "Diese Gruppe wird schon eingerichtet." };
      }
      if (bundle.session.status !== "setup") {
    ...
    try {
      ...
      if (!bundle) {
        return res.status(500).json({
          error: "Gruppe wurde angelegt, konnte aber nicht geladen werden."
        });
      }
      res.json({ success: true, ...bundle });
    } catch (err) {
      ...
      res.status(500).json({
        error: "Serverfehler",
        message: "Die Gruppenarbeit konnte nicht gestartet werden. Bitte versuche es noch einmal."
      });
    }
  });
