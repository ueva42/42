/**
 * Gruppenmodus – API-Routen (Lehrer + Schüler).
 * Gerätekonzept: shared (ein Gerät wird weitergegeben) oder personal (Tabletklasse).
 */
import {
...
export function registerGroupModeRoutes(app, deps) {
  ...
  async function loadSessionBundle(sessionId, schoolId) {
    ...
    const members = membersRes.rows.map((m) => {
      ...
      return {
        ...
        inviteStatus: normalizeInviteStatus(m.invite_status),
  ...
  function summarizeOpenSession(bundle, studentId) {
    const uid = Number(studentId);
    const accepted = acceptedMembers(bundle.members || []);
    const me = (bundle.members || []).find((m) => Number(m.userId) === uid) || null;
    const inviteStatus = me ? normalizeInviteStatus(me.inviteStatus) : null;
    const personal = isPersonalDeviceMode(bundle.settings);
    const maxMembers = Number(bundle.settings?.maxMembers) || 4;
    const minMembers = Number(bundle.settings?.minMembers) || 2;
    const isHost = Number(bundle.session.hostUserId) === uid;
    const isMine = isHost || inviteStatus === "accepted";
    const pendingForMe = inviteStatus === "pending";
    const setupStep = String(bundle.session.setupStep || "members");
    const canJoin =
      personal &&
      !me &&
      bundle.session.status === "setup" &&
      (setupStep === "members" || setupStep === "roles") &&
      accepted.length < maxMembers;
    return {
      ...
      memberNames: accepted.map((m) => m.displayName).filter(Boolean),
      memberCount: accepted.length,
      minMembers,
      maxMembers,
      deviceMode: personal ? "personal" : "shared",
      isMine,
      pendingForMe,
      canJoin
    };
  ...
  async function addMemberToSession(sessionId, userId, classId, inviteStatus = "accepted") {
    const user = await pool.query(
      `
      SELECT id, name FROM users
      WHERE id = $1 AND role = 'student' AND class_id = $2
    `,
      [userId, classId]
    );
    if (!user.rows[0]) {
      return { ok: false, message: "Person nicht in der Klasse." };
    }
    const orderRes = await pool.query(
      `SELECT COALESCE(MAX(sort_order), 0) + 1 AS n FROM group_session_members WHERE session_id = $1`,
      [sessionId]
    );
    await pool.query(
      `
      INSERT INTO group_session_members (
        session_id, user_id, display_name_snapshot, sort_order, invite_status
      )
      VALUES ($1, $2, $3, $4, $5)
      ON CONFLICT (session_id, user_id) DO UPDATE
        SET invite_status = CASE
              WHEN group_session_members.invite_status = 'accepted' THEN 'accepted'
    ...
    `,
      [
        sessionId,
        userId,
        displayNameFromUser(user.rows[0].name),
        orderRes.rows[0].n,
        normalizeInviteStatus(inviteStatus)
      ]
    );
    return { ok: true };
  }
    ...
    try {
      ...
      `,
        [
          ...
          existing.id,
          maxHow,
          normalizeDeviceMode(req.body.deviceMode)
        ]
      );
  ...
  app.get("/api/student/group-mode/bootstrap", isStudent, async (req, res) => {
    try {
      ...
      for (const row of activeRes.rows) {
        try {
          ...
          if (bundle) {
            ...
            const summary = summarizeOpenSession(bundle, studentId);
            activeSessions.push(summary);
            if (summary.pendingForMe) pendingInvites.push(summary);
            else if (summary.canJoin) joinableGroups.push(summary);
          } else {
            await deleteSessionCascade(row.id);
          }
        } catch (sessionErr) {
      ...
      // Geschlossene Gruppen als feste Stammgruppen (für „neue Stunde / neues Ziel“)
      const fixedGroups = [];
      const openSubjects = new Set(
        activeSessions.map((s) => String(s.subject || "").toLowerCase().trim())
      );
      try {
        const closedRes = await pool.query(
          ...
          SELECT gs.id
          FROM group_sessions gs
          JOIN group_session_members gsm
            ON gsm.session_id = gs.id AND gsm.user_id = $2
          WHERE gs.class_id = $1
            AND gs.status = 'closed'
            AND EXISTS (
              SELECT 1 FROM group_session_member_roles r WHERE r.member_id = gsm.id
            )
          ORDER BY gs.closed_at DESC NULLS LAST, gs.updated_at DESC NULLS LAST
          LIMIT 20
        `,
          [classId, studentId]
        );
        ...
        for (const row of closedRes.rows) {
          try {
            const bundle = await loadSessionBundle(row.id, schoolId);
            if (!bundle) continue;
            const key = String(bundle.session.subject || "").toLowerCase().trim();
            if (!key || openSubjects.has(key) || seenSubjects.has(key)) continue;
        ...
        console.error("⚠️ bootstrap fixedGroups:", fixedErr);
      }
  ...
  app.post("/api/student/group-sessions", isStudent, async (req, res) => {
    try {
      ...
      if (settingsRow && isPersonalDeviceMode(serializeSettingsRow(settingsRow, []))) {
        try {
          await addMemberToSession(ins.rows[0].id, studentId, classId, "accepted");
        } catch (addErr) {
          console.error("⚠️ personal host member:", addErr);
        }
      }
  ...
  async function respondWithSessionBundle(res, sessionId, schoolId, extra = {}) {
    const bundle = await loadSessionBundle(sessionId, schoolId);
    if (!bundle) {
      return res.status(500).json({
        error: "Serverfehler",
        message: "Gruppe konnte nicht geladen werden."
      });
    }
    return res.json({ success: true, ...bundle, ...extra });
  }

  app.post("/api/student/group-sessions/:id/join", isStudent, async (req, res) => {
    try {
      const studentId = req.session.user.id;
      const access = await requireEditableSession(studentId, req.params.id);
      if (!access.ok) return res.status(access.status).json({ error: access.error });
      if (access.bundle.session.status !== "setup") {
        return res.json({ success: false, message: "Dieser Gruppe kann man jetzt nicht mehr beitreten." });
      }
      const setupStep = String(access.bundle.session.setupStep || "members");
      if (setupStep !== "members" && setupStep !== "roles") {
        return res.json({
          success: false,
          message: "Die Gruppe hat schon mit dem Thema begonnen."
        });
      }
      if (!isPersonalDeviceMode(access.bundle.settings)) {
        return res.json({
          success: false,
          message: "Beitreten geht nur, wenn jede Person ein eigenes Gerät nutzt."
        });
      }

      const accepted = acceptedMembers(access.bundle.members);
      const me = access.bundle.members.find((m) => Number(m.userId) === Number(studentId));
      if (me && isAcceptedMember(me)) {
        return respondWithSessionBundle(res, access.bundle.session.id, access.schoolId);
      }

      const check = validateMemberCount(accepted.length + (me ? 0 : 1), access.bundle.settings);
      if (!check.ok) return res.json({ success: false, message: check.message });
      ...
      if (busy.has(Number(studentId))) {
        return res.json({
          success: false,
          message: "Du bist schon in einer anderen Gruppe für dieses Fach."
        });
      }

      const added = await addMemberToSession(
        access.bundle.session.id,
        studentId,
        access.classId,
        "accepted"
      );
      if (!added.ok) return res.json({ success: false, message: added.message });

      return respondWithSessionBundle(res, access.bundle.session.id, access.schoolId);
    } catch (err) {
      console.error("❌ POST join:", err);
      res.status(500).json({ error: "Serverfehler", message: "Beitreten fehlgeschlagen." });
    }
  });

  app.post("/api/student/group-sessions/:id/leave", isStudent, async (req, res) => {
    try {
      const studentId = req.session.user.id;
      const access = await requireEditableSession(studentId, req.params.id);
      if (!access.ok) return res.status(access.status).json({ error: access.error });
      if (access.bundle.session.status !== "setup") {
        return res.json({ success: false, message: "Die Gruppe ist schon gestartet." });
      }
      const me = access.bundle.members.find((m) => Number(m.userId) === Number(studentId));
      if (!me) return res.json({ success: true });

      await pool.query("DELETE FROM group_session_member_roles WHERE member_id = $1", [me.id]);
      await pool.query("DELETE FROM group_session_members WHERE id = $1", [me.id]);

      const remaining = acceptedMembers(access.bundle.members).filter(
      ...
      if (Number(access.bundle.session.hostUserId) === Number(studentId)) {
        await pool.query(
          `UPDATE group_sessions SET host_user_id = $2, updated_at = NOW() WHERE id = $1`,
          [access.bundle.session.id, remaining[0].userId]
        );
      }
      return respondWithSessionBundle(res, access.bundle.session.id, access.schoolId);
    } catch (err) {
      console.error("❌ POST leave:", err);
      res.status(500).json({ error: "Serverfehler", message: "Verlassen fehlgeschlagen." });
    }
  });

  app.post("/api/student/group-sessions/:id/invite", isStudent, async (req, res) => {
    try {
      const studentId = req.session.user.id;
      const access = await requireEditableSession(studentId, req.params.id);
      if (!access.ok) return res.status(access.status).json({ error: access.error });
      if (access.bundle.session.status !== "setup") {
        return res.json({ success: false, message: "Einladen geht nur beim Einrichten." });
      }
      const setupStep = String(access.bundle.session.setupStep || "members");
      if (setupStep !== "members" && setupStep !== "roles") {
        return res.json({
      ...
      if (!isPersonalDeviceMode(access.bundle.settings)) {
        return res.json({ success: false, message: "Einladen gilt nur für Tabletklassen." });
      }
      const me = access.bundle.members.find((m) => Number(m.userId) === Number(studentId));
      const isHost = Number(access.bundle.session.hostUserId) === Number(studentId);
      if (!isHost && !(me && isAcceptedMember(me))) {
        return res.status(403).json({ error: "Nur Gruppenmitglieder können einladen." });
      }

      const targetId = Number(req.body.userId);
      if (!Number.isFinite(targetId) || targetId <= 0) {
        return res.json({ success: false, message: "Bitte eine Person wählen." });
      }
      if (Number(targetId) === Number(studentId)) {
        return res.json({ success: false, message: "Du bist schon in der Gruppe." });
      }

      const accepted = acceptedMembers(access.bundle.members);
      const already = access.bundle.members.find((m) => Number(m.userId) === targetId);
      if (already && isAcceptedMember(already)) {
        return respondWithSessionBundle(res, access.bundle.session.id, access.schoolId);
      }

      const seats = accepted.length + (already ? 0 : 1);
      const check = validateMemberCount(
        Math.max(seats, access.bundle.settings.minMembers),
        access.bundle.settings
      );
      if (seats > (access.bundle.settings.maxMembers || 4)) {
        return res.json({
          success: false,
          message: `Maximal ${access.bundle.settings.maxMembers} Personen pro Gruppe.`
        });
      }
      if (!check.ok && seats > access.bundle.settings.maxMembers) {
      ...
      if (busy.has(targetId)) {
        return res.json({
          success: false,
          message: "Die Person ist schon in einer anderen Gruppe."
        });
      }
      ...
      res.status(500).json({ error: "Serverfehler", message: "Einladen fehlgeschlagen." });
    }
  });

  app.post("/api/student/group-sessions/:id/invite-respond", isStudent, async (req, res) => {
    try {
      const studentId = req.session.user.id;
      const access = await requireEditableSession(studentId, req.params.id);
      if (!access.ok) return res.status(access.status).json({ error: access.error });
      const me = access.bundle.members.find((m) => Number(m.userId) === Number(studentId));
      if (!me) {
        return res.json({ success: false, message: "Keine Einladung gefunden." });
      }
      ...
      if (isAcceptedMember(me)) {
        return respondWithSessionBundle(res, access.bundle.session.id, access.schoolId);
      }
      const accepted = acceptedMembers(access.bundle.members);
      const check = validateMemberCount(accepted.length + 1, access.bundle.settings);
      if (accepted.length + 1 > (access.bundle.settings.maxMembers || 4)) {
        return res.json({ success: false, message: check.message || "Die Gruppe ist voll." });
      }
      await pool.query(
        `UPDATE group_session_members SET invite_status = 'accepted', updated_at = NOW() WHERE id = $1`,
        [me.id]
      );
      return respondWithSessionBundle(res, access.bundle.session.id, access.schoolId);
    } catch (err) {
      console.error("❌ POST invite-respond:", err);
      res.status(500).json({ error: "Serverfehler", message: "Antwort fehlgeschlagen." });
    }
  });

  app.post("/api/student/group-sessions/:id/members/ready", isStudent, async (req, res) => {
    try {
      const access = await requireEditableSession(req.session.user.id, req.params.id);
      if (!access.ok) return res.status(access.status).json({ error: access.error });
      if (access.bundle.session.status !== "setup") {
      ...
      const accepted = acceptedMembers(access.bundle.members);
      const check = validateMemberCount(accepted.length, access.bundle.settings);
      if (!check.ok) return res.json({ success: false, message: check.message });
      await pool.query(
        `UPDATE group_sessions SET setup_step = 'roles', updated_at = NOW() WHERE id = $1`,
        [access.bundle.session.id]
      );
      ...
      console.error("❌ POST members/ready:", err);
      res.status(500).json({ error: "Serverfehler" });
    }
  });
  ...
  app.patch("/api/student/group-sessions/:id/roles", isStudent, async (req, res) => {
    try {
      ...
      const roles = (access.bundle.settings?.roles || []).filter((r) => r && r.active !== false);
      const personal = isPersonalDeviceMode(access.bundle.settings);

      let assignments = Array.isArray(req.body.assignments) ? req.body.assignments : [];
      if (personal) {
        assignments = [];
        for (const m of acceptedMembers(access.bundle.members)) {
          for (const r of m.roles || []) {
            if (r.roleId) assignments.push({ userId: m.userId, roleId: r.roleId });
          }
        }
      }
      ...
      const memberByUser = new Map(
        acceptedMembers(access.bundle.members).map((m) => [String(m.userId), m])
      );
  ...
  app.patch("/api/student/group-sessions/:id/roles/claim", isStudent, async (req, res) => {
    try {
      const studentId = req.session.user.id;
      const access = await requireEditableSession(studentId, req.params.id);
      if (!access.ok) return res.status(access.status).json({ error: access.error });
      ...
      if (!isPersonalDeviceMode(access.bundle.settings)) {
        return res.json({ success: false, message: "Rollen selbst übernehmen gilt nur für Tabletklassen." });
      }
      const me = acceptedMembers(access.bundle.members).find(
        (m) => Number(m.userId) === Number(studentId)
      );
      if (!me) {
        return res.json({ success: false, message: "Tritt der Gruppe zuerst bei." });
      }

      const roleId = String(req.body.roleId || "").trim();
      const claim = req.body.claim !== false && req.body.claim !== "false";
      const roles = (access.bundle.settings?.roles || []).filter((r) => r && r.active !== false);
      const role = roles.find((r) => String(r.id) === roleId);
      if (!role) return res.json({ success: false, message: "Rolle nicht gefunden." });

      const holder = acceptedMembers(access.bundle.members).find((m) =>
        (m.roles || []).some((r) => String(r.roleId) === roleId)
      );
      if (claim) {
        if (holder && Number(holder.userId) !== Number(studentId)) {
          return res.json({
            success: false,
            message: `${holder.displayName} hat „${role.name}“ schon übernommen.`
          });
        }
        if (!access.bundle.settings.allowMultiRoles) {
          await pool.query("DELETE FROM group_session_member_roles WHERE member_id = $1", [me.id]);
        }
        if (!holder) {
          await pool.query(
          ...
          `,
            [me.id, role.id, role.name, role.description || ""]
          );
        }
      } else {
        await pool.query(
          `DELETE FROM group_session_member_roles WHERE member_id = $1 AND role_id = $2`,
          [me.id, role.id]
        );
      }
      ...
      console.error("❌ PATCH roles/claim:", err);
      res.status(500).json({ error: "Serverfehler" });
    }
  });
  ...
  app.patch("/api/student/group-sessions/:id/topic", isStudent, async (req, res) => {
    try {
      ...
      // Nur wenn DIESE Session noch keine Mitglieder hat (leere Hülle)
      ...
      await pool.query(
        ...
        UPDATE group_sessions
        SET topic_id = $2,
            topic_name_snapshot = $3,
            setup_step = $4,
            shared_goal = CASE WHEN $4 = 'shared_goal' THEN NULL ELSE shared_goal END,
            updated_at = NOW()
        WHERE id = $1
      `,
        [access.bundle.session.id, topic.id, topic.name, nextStep]
      );

      // Persönliche Ziele für neue Stunde zurücksetzen, wenn Gruppe schon Ziele hatte
      if (currentMembers.length) {
        await pool.query(
          `
          UPDATE group_session_members
          SET what_goal_id = NULL,
              what_goal_text_snapshot = NULL,
              what_goal_area_snapshot = NULL,
              selected_level = NULL,
              level_goal_text_snapshot = NULL,
              how_goal_text = NULL,
  ...
  app.post("/api/student/group-sessions/:id/new-lesson", isStudent, async (req, res) => {
    try {
      const studentId = req.session.user.id;
      const { classId, schoolId } = await getStudentClassContext(studentId);
      if (!classId) {
      ...
      const source = await loadSessionBundle(req.params.id, schoolId);
      if (!source || Number(source.session.classId) !== Number(classId)) {
        return res.status(404).json({ error: "Gruppe nicht gefunden." });
      }

      const isMember = (source.members || []).some((m) => Number(m.userId) === Number(studentId));
      const isHost = Number(source.session.hostUserId) === Number(studentId);
      if (!isMember && !isHost) {
        return res.status(403).json({ error: "Nur Gruppenmitglieder können eine neue Stunde starten." });
      }

      // Offene Session desselben Fachs nicht doppelt
      const openId = await findOpenSessionForStudent({
        classId,
        schoolId,
        studentId,
      ...
      if (openId) {
        const open = await loadSessionBundle(openId, schoolId);
        if (open) {
          return res.json({
            success: true,
            resumed: true,
            message: "Es gibt schon eine offene Stunde – dort weiterarbeiten.",
            ...open
          });
        }
      }
      ...
      const ins = await pool.query(
        ...
        VALUES ($1,$2,$3,$4::date,$5,NULL,NULL,$6,'setup','topic',$7,$7)
        RETURNING id
      `,
        [
          schoolId,
      ...
      for (const m of source.members || []) {
        const memIns = await pool.query(
          ...
          RETURNING id
        `,
          [newId, m.userId, m.displayName || "Schüler", order++]
        );
        const memberId = memIns.rows[0].id;
        for (const role of m.roles || []) {
          ...
          `,
            [memberId, role.roleId || null, role.name || "", role.description || ""]
          );
        }
      }

      const bundle = await loadSessionBundle(newId, schoolId);
      res.json({ success: true, newLesson: true, ...bundle });
    } catch (err) {
      console.error("❌ POST new-lesson:", err);
      res.status(500).json({
        error: "Serverfehler",
        message: err?.message || "Neue Stunde konnte nicht gestartet werden."
      });
    }
  });
  ...
  app.put("/api/student/group-sessions/:id/member-goals", isStudent, async (req, res) => {
    try {
      ...
      const userId = isPersonalDeviceMode(access.bundle.settings)
        ? Number(req.session.user.id)
        : Number(req.body.userId);
      const member = access.bundle.members.find((m) => Number(m.userId) === Number(userId));
      if (!member) {
  ...
  app.put("/api/student/group-sessions/:id/mid-check", isStudent, async (req, res) => {
    try {
      ...
      const focusesRaw = Array.isArray(req.body.changeFocuses)
        ? req.body.changeFocuses
        : Array.isArray(req.body.changeFocus)
          ? req.body.changeFocus
          : req.body.changeFocus
            ? [req.body.changeFocus]
            : [];
      const focuses = focusesRaw
        .map((s) => String(s || "").trim())
        .filter(Boolean)
        .slice(0, 3);
      const strategies = Array.isArray(req.body.changeStrategies)
        ? req.body.changeStrategies.map((s) => String(s || "").trim()).filter(Boolean).slice(0, 1)
        : [];
      const changeStrategy =
        String(req.body.changeStrategy || "").trim() || strategies[0] || null;

      const payload = {
        onTrack: String(req.body.onTrack || "").trim(),
        progress: String(req.body.progress || "").trim(),
        changeNeeded: String(req.body.changeNeeded || "").trim(),
        changeFocus: focuses.length ? focuses.join(" · ") : null,
        changeFocuses: focuses,
        changeStrategy,
        changeStrategies: changeStrategy ? [changeStrategy] : [],
        note: String(req.body.note || "").trim().slice(0, 400) || changeStrategy || null
      };
