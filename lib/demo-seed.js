...
async function syncDemoCharacters(pool, schoolId) {
  const sourceChars = await fetchSourceCharacters(pool, DEMO_CHARACTER_SOURCE_SLUG);
  ...
  if (Number(charCount.count) > 0) {
    const existing = await pool.query(
      "SELECT image_url FROM characters WHERE school_id = $1 LIMIT 1",
      [schoolId]
    );
    if (existing.rows.length && !isPlaceholderCharacterImage(existing.rows[0].image_url)) {
