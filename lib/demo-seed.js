...
const TIMETABLE_SLOTS = [
  ...
  { timeslot: "9.30-10.15", subject: "Physik", room: "Lab 1" }
];

const DEMO_DEUTSCH_KA_TOPIC = "Erörterung schreiben";
  ...
  rows: [
    {
      ...
      unterthema: "Startklar",
      rookie: "Ich kenne die Voraussetzungen für den Satz des Pythagoras.",
      operator: "Ich prüfe in Aufgaben, ob der Satz anwendbar ist.",
      streetLegend: "Ich erkläre anderen, wann der Satz gilt und wann nicht.",
      materialType: "url",
      practiceUrl: "https://forschendphaenomenal.de/9pystart/"
    },
    {
      fach: "Mathe",
      thema: "Satz des Pythagoras",
      unterthema: "Satz des Pythagoras",
      rookie: "Ich kenne die Formel a² + b² = c² und setze sie in einfachen Aufgaben ein.",
      operator: "Ich berechne fehlende Seitenlängen selbstständig.",
      streetLegend: "Ich prüfe Ergebnisse und erkläre meinen Rechenweg.",
      materialType: "url",
      practiceUrl: "https://forschendphaenomenal.de/9py1ut/"
    },
    {
      fach: "Mathe",
      thema: "Satz des Pythagoras",
      unterthema: "Hypotenuse und Kathete",
      rookie: "Ich kann im rechtwinkligen Dreieck Hypotenuse und Katheten benennen.",
      operator: "Ich ordne die Seiten in Skizzen und Aufgaben korrekt zu.",
      streetLegend: "Ich begründe sicher, welche Seite die Hypotenuse ist.",
      materialType: "url",
      practiceUrl: "https://forschendphaenomenal.de/9py2ut/"
    }
  ]
};

/** Erste drei Ziele aus dem Optik-I-Kompetenzraster (Physik Klasse 7). */
const CATALOG_OPTIK = {
  name: "Optik I",
  gradeLevel: "7",
  rows: [
    {
      fach: "Physik",
      thema: "Optik I",
      unterthema: "Lichtquellen und Sehvorgang",
      rookie:
        "Ich kann Lichtsender (Sonne, Lampe) und Lichtempfänger (Auge) benennen und natürliche von künstlichen Lichtquellen unterscheiden.",
      operator:
        "Ich kann erklären, dass Licht vom Sender über den Gegenstand bis zum Auge gelangen muss, damit ich etwas sehe.",
      streetLegend:
        "Ich kann erklären, warum ich in einem völlig dunklen Raum nichts sehe, obwohl die Gegenstände ja da sind.",
      materialType: "url",
      practiceUrl: "https://forschendphaenomenal.de/p7optik1/"
    },
    {
      fach: "Physik",
      thema: "Optik I",
      unterthema: "Ausbreitung von Licht I – in alle Richtungen",
      rookie: "Ich kann zeigen, dass sich Licht von einer Lampe in alle Richtungen ausbreitet.",
      operator:
        "Ich kann erklären, warum man eine eingeschaltete Lampe von jedem Platz im Raum aus sehen kann.",
      streetLegend: "Ich kann erklären, warum eine einzige Deckenlampe den ganzen Raum erhellt.",
      materialType: "url",
      practiceUrl: "https://forschendphaenomenal.de/p7optik1/"
    },
    {
      fach: "Physik",
      thema: "Optik I",
      unterthema: "Ausbreitung von Licht II – geradlinig",
      rookie:
        "Ich kann zeigen, dass sich Licht geradlinig ausbreitet, und es mit geraden Strahlen darstellen.",
      operator:
        "Ich kann mit der geradlinigen Ausbreitung erklären, warum man ohne Hilfsmittel nicht „um die Ecke“ leuchten kann.",
      streetLegend:
        "Ich kann erklären, wie man mit Spiegeln (z. B. einem Periskop) trotzdem um eine Ecke oder über eine Mauer sehen kann.",
      materialType: "url",
      practiceUrl: "https://forschendphaenomenal.de/p7optik1/"
    }
  ]
...
function berlinDateAdd(dateStr, offsetDays) {
  const [y, m, day] = dateStr.split("-").map(Number);
  const utc = new Date(Date.UTC(y, m - 1, day, 12, 0, 0));
  utc.setUTCDate(utc.getUTCDate() + offsetDays);
  return new Intl.DateTimeFormat("en-CA", { timeZone: "Europe/Berlin" }).format(utc);
}

/** Demo läuft Mo–Fr: Sa/So werden auf Freitag gemappt, damit Stundenplan & Logbuch nutzbar bleiben. */
export function demoEffectiveIsoDate(dateStr) {
  const base = dateStr || berlinDateString(0);
  const wd = berlinWeekday(base);
  if (wd <= 5) return base;
  return berlinDateAdd(base, wd === 6 ? -1 : -2);
}
...
async function deleteLevelCheckCascade(pool, levelCheckId) {
  if (!levelCheckId) return;
  await pool.query(`DELETE FROM level_check_checkpoints WHERE level_check_id = $1`, [
    levelCheckId
  ]);
  await pool.query(`DELETE FROM level_check_goals WHERE level_check_id = $1`, [levelCheckId]);
  await pool.query(`DELETE FROM level_checks WHERE id = $1`, [levelCheckId]);
}

async function replaceTopicGoals(pool, schoolId, levelCheckId, rows) {
  await pool.query(`DELETE FROM level_check_goals WHERE level_check_id = $1`, [levelCheckId]);
  const goalIds = [];
...
/**
 * Demo-Levelplan Mathe: nur Pythagoras mit den drei verknüpften Unterthemen.
 * Entfernt u. a. Wahrscheinlichkeit und alte Unterthemen.
 */
async function syncDemoMatheCatalog(pool, schoolId, classId) {
  const assigned = await pool.query(
    `
    SELECT catalog_id
    FROM class_level_plan_assignments
    WHERE class_id = $1 AND lower(trim(subject)) = 'mathe'
  ...
  for (const row of assigned.rows) {
    const hasPyth = await pool.query(
      `
      SELECT 1 FROM level_checks
      WHERE catalog_id = $1 AND lower(trim(name)) LIKE '%pythagoras%'
      LIMIT 1
    `,
      [row.catalog_id]
    );
    if (hasPyth.rows.length) {
      catalogId = row.catalog_id;
      break;
    }
  }
  if (!catalogId) catalogId = assigned.rows[0]?.catalog_id || null;
  if (!catalogId) {
    const byName = await pool.query(
      `
      SELECT id FROM level_plan_catalogs
      WHERE school_id = $1 AND lower(trim(name)) LIKE '%pythagoras%'
      ORDER BY created_at ASC
      LIMIT 1
    `,
      [schoolId]
    );
    catalogId = byName.rows[0]?.id || null;
  }

  if (!catalogId) {
    const created = await createCatalog(pool, schoolId, CATALOG_PYTHAGORAS);
    ...
    return {
      ...
      topicId: created.topics?.[0]?.id || null
    };
  }
  ...
  await pool.query(
    ...
    SET name = $2, grade_level = $3
    WHERE id = $1
  `,
    [catalogId, CATALOG_PYTHAGORAS.name.slice(0, 120), CATALOG_PYTHAGORAS.gradeLevel]
  );
  ...
  let pythTopicId =
    topics.rows.find((t) => /pythagoras/i.test(String(t.name || "")))?.id || null;

  for (const topic of topics.rows) {
    if (pythTopicId && topic.id === pythTopicId) continue;
    if (!pythTopicId && /pythagoras/i.test(String(topic.name || ""))) {
      pythTopicId = topic.id;
      continue;
    }
    await deleteLevelCheckCascade(pool, topic.id);
  }

  const wahrsche = await pool.query(
    `
    SELECT lc.id
    FROM level_checks lc
    WHERE lc.school_id = $1
      AND lower(trim(lc.subject)) = 'mathe'
      AND lower(trim(lc.name)) LIKE '%wahrscheinlichkeit%'
  `,
    [schoolId]
  );
  for (const row of wahrsche.rows) {
    await deleteLevelCheckCascade(pool, row.id);
  }

  if (!pythTopicId) {
    pythTopicId = await findOrCreateCatalogTopic(
      pool,
      catalogId,
      schoolId,
      "Mathe",
      CATALOG_PYTHAGORAS.name
    );
  } else {
    await pool.query(
      `
      UPDATE level_checks
      SET name = $2, subject = 'Mathe', sort_order = 1
      WHERE id = $1
    `,
      [pythTopicId, CATALOG_PYTHAGORAS.name.slice(0, 120)]
    );
  }

  const goalIds = await replaceTopicGoals(
    pool,
    schoolId,
    pythTopicId,
    CATALOG_PYTHAGORAS.rows
  );
  return { catalogId, goalIds, topicId: pythTopicId };
}

/**
 * Demo-Levelplan Physik: Optik I mit den ersten drei Kompetenzzielen.
 */
async function syncDemoOptikCatalog(pool, schoolId, classId) {
  const assigned = await pool.query(
    `
    SELECT catalog_id
    FROM class_level_plan_assignments
    WHERE class_id = $1 AND lower(trim(subject)) = 'physik'
    ORDER BY catalog_id ASC
  `,
    [classId]
  );
  ...
  let catalogId = null;
  for (const row of assigned.rows) {
    const hasOptik = await pool.query(
      `
      SELECT 1 FROM level_checks
      WHERE catalog_id = $1 AND lower(trim(name)) LIKE '%optik%'
      LIMIT 1
    `,
      [row.catalog_id]
    );
    if (hasOptik.rows.length) {
      catalogId = row.catalog_id;
      break;
    }
  }
  ...
  if (!catalogId) {
    const byName = await pool.query(
      `
      SELECT id FROM level_plan_catalogs
      WHERE school_id = $1 AND lower(trim(name)) LIKE '%optik%'
      ORDER BY created_at ASC
      LIMIT 1
    `,
      [schoolId]
  ...
  if (!catalogId) {
    const created = await createCatalog(pool, schoolId, CATALOG_OPTIK);
    catalogId = created.catalogId;
    await pool.query(
      `
      INSERT INTO class_level_plan_assignments (class_id, subject, catalog_id)
  ...
  await pool.query(
    ...
    WHERE id = $1
  `,
    [catalogId, CATALOG_OPTIK.name.slice(0, 120), CATALOG_OPTIK.gradeLevel]
  );
  ...
  let optikTopicId =
    topics.rows.find((t) => /optik/i.test(String(t.name || "")))?.id || null;

  for (const topic of topics.rows) {
    if (optikTopicId && topic.id === optikTopicId) continue;
    if (!optikTopicId && /optik/i.test(String(topic.name || ""))) {
      optikTopicId = topic.id;
      continue;
    }
    await deleteLevelCheckCascade(pool, topic.id);
  }
  ...
  if (!optikTopicId) {
    optikTopicId = await findOrCreateCatalogTopic(
      pool,
      catalogId,
      schoolId,
      "Physik",
    ...
    await pool.query(
      `
      UPDATE level_checks
      SET name = $2, subject = 'Physik', sort_order = 1
      WHERE id = $1
    `,
      [optikTopicId, CATALOG_OPTIK.name.slice(0, 120)]
    );
  }

  const goalIds = await replaceTopicGoals(pool, schoolId, optikTopicId, CATALOG_OPTIK.rows);
  return { catalogId, goalIds, topicId: optikTopicId };
}

async function replaceDemoPhysikRoles(pool, schoolId, settingsId) {
  const classRes = await pool.query(
    `SELECT class_id FROM group_mode_settings WHERE id = $1`,
    [settingsId]
  );
  ...
  if (classId) {
    await pool.query(
      ...
      WHERE member_id IN (
        SELECT m.id FROM group_session_members m
        JOIN group_sessions gs ON gs.id = m.session_id
        WHERE gs.class_id = $1 AND lower(trim(gs.subject)) = 'physik'
      )
    `,
      [classId]
    );
  ...
  await pool.query(
    `
    DELETE FROM group_mode_role_goals
    WHERE role_id IN (SELECT id FROM group_mode_roles WHERE settings_id = $1)
  `,
    [settingsId]
  );
  await pool.query(`DELETE FROM group_mode_roles WHERE settings_id = $1`, [settingsId]);
  await ensureDefaultRolesForSettings(pool, schoolId, settingsId, DEFAULT_PHYSICS_ROLES);
}

async function seedDemoPhysikGroupMode(pool, schoolId, classId) {
  const existing = await pool.query(
    `
    SELECT id FROM group_mode_settings
    WHERE class_id = $1 AND lower(trim(subject)) = 'physik'
    LIMIT 1
  `,
    [classId]
  ...
  let settingsId = existing.rows[0]?.id || null;
  if (settingsId) {
    await pool.query(
      `
      UPDATE group_mode_settings SET
        ...
        enable_shared_goal = TRUE,
        enable_what_goals = TRUE,
        max_what_goals = 3,
        max_how_goals = 3,
        enable_how_goals = TRUE,
        enable_mid_check = TRUE,
        enable_reflection = TRUE,
        allow_free_what_goal = FALSE,
        allow_free_how_goal = TRUE,
        how_goal_options = $2::jsonb,
        updated_at = NOW()
      WHERE id = $1
    `,
      [settingsId, JSON.stringify(DEFAULT_GROUP_HOW_GOALS)]
    );
  } else {
    const ins = await pool.query(
      `
      INSERT INTO group_mode_settings (
        ...
        enable_shared_goal, enable_what_goals, max_what_goals, max_how_goals,
      ...
      RETURNING id
    `,
      [schoolId, classId, JSON.stringify(DEFAULT_GROUP_HOW_GOALS)]
    );
    settingsId = ins.rows[0].id;
  }

  await replaceDemoPhysikRoles(pool, schoolId, settingsId);
  return { settingsId };
}

async function seedDemoLinaPythagorasMarks(pool, schoolId, classId, goalIds) {
  if (!(goalIds || []).length) return;
  const lina = await pool.query(
    `
    SELECT id FROM users
    WHERE school_id = $1 AND class_id = $2 AND name = $3 AND role = 'student'
    LIMIT 1
  `,
    [schoolId, classId, DEMO_STUDENT]
  );
  const linaId = lina.rows[0]?.id;
  if (!linaId) return;
...
async function ensureDemoDeutschTopic(pool, schoolId, classId) {
  const existing = await pool.query(
    `
    SELECT id FROM level_checks
    WHERE class_id = $1 AND school_id = $2
      AND lower(trim(subject)) = 'deutsch'
  ...
  const ins = await pool.query(
    ...
    VALUES ($1, $2, 'Deutsch', $3, 1)
    RETURNING id
  `,
    [schoolId, classId, DEMO_DEUTSCH_KA_TOPIC]
  );
  return ins.rows[0].id;
}

async function seedDemoMatheCheckpoints(pool, schoolId, classId, topicId, goalIds) {
  if (!topicId) return;
  ...
  const linked = (goalIds || []).slice(0, 3);
  const levelcheckDate = berlinDateString(5);
  const klassenarbeitDate = berlinDateString(12);
...
async function seedDemoDeutschCheckpoint(pool, schoolId, classId, topicId) {
  if (!topicId) return;
  ...
  const klassenarbeitDate = berlinDateString(14);
  ...
  await pool.query(
    ...
    VALUES ($1, $2, $3, $4, 'klassenarbeit', NULL, '[]'::jsonb)
  `,
    [schoolId, topicId, classId, klassenarbeitDate]
  );
}
...
export async function refreshDemoLearningSetup(pool, schoolId) {
  ...
  const mathe = await syncDemoMatheCatalog(pool, schoolId, classId);
  if (mathe.topicId) {
    await seedDemoMatheCheckpoints(pool, schoolId, classId, mathe.topicId, mathe.goalIds);
  }
  await seedDemoLinaPythagorasMarks(pool, schoolId, classId, mathe.goalIds);
  await syncDemoOptikCatalog(pool, schoolId, classId);
  await seedDemoPhysikGroupMode(pool, schoolId, classId);

  const deutschTopicId = await ensureDemoDeutschTopic(pool, schoolId, classId);
  await seedDemoDeutschCheckpoint(pool, schoolId, classId, deutschTopicId);
...
export async function resetDemoSchool(pool) {
  ...
  try {
    ...
    await client.query(
      ...
      WHERE member_id IN (
        ...
        WHERE gs.school_id = $1
          OR gs.class_id IN (SELECT id FROM classes WHERE school_id = $1)
      )
    `,
      [schoolId]
    );
    await client.query(
      `
      DELETE FROM group_session_docs
      WHERE session_id IN (
    ...
    await client.query(
      `
      DELETE FROM group_competency_activity
      WHERE session_id IN (
        SELECT id FROM group_sessions
        WHERE school_id = $1
          OR class_id IN (SELECT id FROM classes WHERE school_id = $1)
    ...
    await client.query(
      ...
      DELETE FROM group_mode_role_goals
      WHERE school_id = $1
        OR role_id IN (SELECT id FROM group_mode_roles WHERE school_id = $1)
    `,
      [schoolId]
    );
    await client.query(
      `
      DELETE FROM group_mode_roles
      WHERE school_id = $1
        OR settings_id IN (
          SELECT id FROM group_mode_settings
          WHERE school_id = $1
    ...
    await client.query(
      `
      DELETE FROM group_mode_settings
      WHERE school_id = $1
        OR class_id IN (SELECT id FROM classes WHERE school_id = $1)
    `,
      [schoolId]
...
export async function seedDemoSchool(pool, options = {}) {
  ...
  const today = demoEffectiveIsoDate(berlinDateString(0));
  const yesterday = demoEffectiveIsoDate(berlinDateAdd(today, -1));
  const sampleLogs = [
    {
      ...
      timeslot: "7.50-8.35",
      subject: "Mathe",
      goal: "Für die Klassenarbeit: Pythagoras-Aufgaben üben",
      confidence: 4,
    ...
    },
    {
      ...
      timeslot: "8.40-9.25",
      subject: "Deutsch",
      goal: "Erörterung: Einleitung für die Klassenarbeit formulieren",
      confidence: 3
    },
    {
      studentName: DEMO_STUDENT,
    ...
    },
    {
      ...
      timeslot: "8.40-9.25",
      subject: "Deutsch",
      goal: "Argumente sammeln für die Erörterung",
      confidence: 4
    }
  ];
