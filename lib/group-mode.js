...
/** shared = ein Gerät wird weitergegeben; personal = Tabletklasse, jedes eigene Gerät. */
export function normalizeDeviceMode(value) {
  const v = String(value || "")
    .toLowerCase()
    .trim();
  if (v === "personal" || v === "own" || v === "1:1" || v === "tabletklasse") {
    return "personal";
  }
  return "shared";
}

export function isPersonalDeviceMode(settingsOrValue) {
  if (settingsOrValue && typeof settingsOrValue === "object") {
    return (
      normalizeDeviceMode(settingsOrValue.deviceMode || settingsOrValue.device_mode) ===
      "personal"
    );
  }
  return normalizeDeviceMode(settingsOrValue) === "personal";
}
...
export function normalizeInviteStatus(value) {
  const v = String(value || "")
    .toLowerCase()
    .trim();
  return v === "pending" ? "pending" : "accepted";
}

export function isAcceptedMember(member) {
  return (
    normalizeInviteStatus(member?.inviteStatus || member?.invite_status) === "accepted"
  );
}

export function acceptedMembers(list) {
  return (list || []).filter(isAcceptedMember);
}
...
export function memberGoalsComplete(member, settings = {}) {
  ...
  if (enableWhat) {
    const whatList = normalizeGoalList(
      member?.what_goals_json || member?.whatGoals
    );
    const hasGoal =
      whatList.length > 0 ||
      !!member?.what_goal_id ||
  ...
  if (enableHow) {
    const howList = normalizeGoalList(member?.how_goals_json || member?.howGoals);
    const how =
      howList.length > 0
        ? "ok"
        : String(member?.how_goal_text || member?.howGoalText || "").trim();
    if (!how) return false;
  }
  return !!(member?.goals_confirmed_at || member?.goalsConfirmedAt);
}

/** Normalisiert gespeicherte Ziel-Listen (Array oder Legacy-Einzelwert). */
export function normalizeGoalList(value) {
  if (Array.isArray(value)) {
    return value
      .map((g) => {
        ...
        if (!text && !g.id) return null;
        return {
          id: g.id || null,
          text: text || String(g.text || ""),
          roleName: g.roleName || g.role_name || null
        };
      })
      .filter(Boolean);
  }
...
export function joinGoalTexts(list) {
  return normalizeGoalList(list)
    .map((g) => g.text)
    .filter(Boolean)
    .join(" · ");
}
...
export function sessionProgress(members, settings = {}) {
  const list = acceptedMembers(members);
...
export function serializeSettingsRow(row, roles = []) {
  ...
  return {
    ...
    subject: row.subject,
    enabled: !!row.enabled,
    deviceMode: normalizeDeviceMode(row.device_mode),
    ...
    maxWhatGoals: Math.min(3, Math.max(1, Number(row.max_what_goals) || 3)),
    maxHowGoals: Math.min(3, Math.max(1, Number(row.max_how_goals) || 3)),
...
export async function migrateGroupModeTables(pool) {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS group_mode_settings (
      ...
      max_what_goals INTEGER NOT NULL DEFAULT 3,
  ...
  await ensureCol("group_session_members", "what_goals_json", "JSONB DEFAULT '[]'::jsonb");
  await ensureCol("group_session_members", "how_goals_json", "JSONB DEFAULT '[]'::jsonb");
  await ensureCol("group_mode_settings", "max_how_goals", "INTEGER NOT NULL DEFAULT 3");
  await ensureCol("group_mode_settings", "device_mode", "TEXT NOT NULL DEFAULT 'shared'");
  await ensureCol("group_session_members", "invite_status", "TEXT NOT NULL DEFAULT 'accepted'");
  await pool.query(`
    UPDATE group_mode_settings
    SET device_mode = 'shared'
    WHERE device_mode IS NULL OR device_mode NOT IN ('shared', 'personal')
  `);
  await pool.query(`
    UPDATE group_session_members
    SET invite_status = 'accepted'
    WHERE invite_status IS NULL OR invite_status NOT IN ('accepted', 'pending')
  `);
  // Bestehende Setups: Mehrfachauswahl bis 3 freischalten
  await pool.query(`
    UPDATE group_mode_settings
    SET max_what_goals = 3
    WHERE max_what_goals IS NULL OR max_what_goals < 3 OR max_what_goals > 3
  `);
  await pool.query(`
    UPDATE group_mode_settings
    SET max_how_goals = 3
    WHERE max_how_goals IS NULL OR max_how_goals < 3 OR max_how_goals > 3
  `);
