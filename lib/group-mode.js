...
export function serializeSettingsRow(row, roles = []) {
  ...
  return {
    ...
    roles: (roles || []).map(serializeRoleRow).filter(Boolean)
  };
}
...
/** Verwaiste Mitglieder/Rollen entfernen und alte leere Setup-Hülsen aufräumen. */
export async function repairGroupModeIntegrity(pool, opts = {}) {
  ...
  await pool.query(`
    ...
    WHERE member_id NOT IN (SELECT id FROM group_session_members)
  `);
  await pool.query(`
    DELETE FROM group_session_members
    WHERE session_id NOT IN (SELECT id FROM group_sessions)
  `);
  await pool.query(`
