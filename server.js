...
function parseLevelplanImportText(raw) {
  return parseLevelplanImportTextBase(raw, normalizeImportSubject);
}

function normalizeLevelplanImportRows(rows, subjectOverride = null) {
  return normalizeLevelplanImportRowsBase(rows, subjectOverride, normalizeImportSubject);
}
...
app.post("/api/teacher/levelplan-import/preview", isAdmin, async (req, res) => {
  try {
    ...
    const rows = normalizeLevelplanImportRows(parseLevelplanImportText(text), subjectOverride);
    res.json({
      success: true,
      rows,
      summary: {
